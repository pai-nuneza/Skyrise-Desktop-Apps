...
namespace Payroll.DAL
{
    public class DALHelper : BaseDAL, IDisposable
    {
        ...
        public DALHelper(bool flag, string DataBaseName)
        {
            this.dbConn = this.getConnectionSelectedDB(DataBaseName);
        }
        public DALHelper(string connectionString, int flag)
        {
            if (flag == 1)
            {
                this.dbConn = this.getConnectionFromFullCS(connectionString);
            }
            else
            {
                this.dbConn = this.getConnection();   
        ...
        private SqlConnection getConnectionFromFullCS(string connectionString)
        {
            return new SqlConnection(connectionString);
        }
