...
namespace Posting.BLogic
{
    public class HRCReportsBL : BaseBL
    {
        ...
        public DataTable GetAllowance()
        {
            string query = @"
                    select 
                       Acm_AllowanceCode  [Code],
                       Acm_AllowanceDesc  [Description]
        ...
        public DataTable GetCurrency()
        {
            string query = @"
                    select 
                        Adt_AccountCode as [Code]
                       ,Adt_AccountDesc as [Description]
                       from T_AccountDetail
                       where Adt_AccountType = 'CURRENCY'
                       and Adt_Status = 'A'
                    ";
        ...
        public DataSet getHeaderData()
        {
            ...
            string query = @"
                            ...
                            LEFT Join T_AccountDetail on Ccd_CompanyAddress3 = Adt_AccountCode and Adt_AccountType='ZIPCODE'";
        ...
        //Added by Christian Aloba for Report Access Rights Checking
        public bool GetAccessRights2(string user_Logged, string systemID)
        {
            bool ret = false;
            string query = @"
                select 
        ...
        public bool GetAccessRightsCanGenerate(string user_Logged, string systemID)
        {
            bool ret = false;
            string query = @"
                select 
                ...
                case "Premium Remittance Report":
                    ...
                    if (category)
                    {
                            ...
                            && PremRemittanceType != "ALL")
                        {
                            ...
                            else if (PremRemittanceType.IndexOf("PHPREM") != -1)
                            {
                                table = new string[9] { 
                                "ID Number"
                                ,"Last Name"
        ...
        public string UserCostCenterAccessTmpQuery(string profile, string systemID, string userCode, string costCenterColumn, string jobStatusColumn, string empStatusColumn, bool reportType)
        {
            string query = string.Format(@"
INNER JOIN 
{0}..T_UserCostCenterAccessTmp
ON Uct_SystemID = '{1}'
AND Uct_UserCode = '{2}'
AND Uct_CostCenterCode = {3}
AND Uct_Jobstatus = {4}
AND Uct_EmploymentStatus = {5}"
                , profile, systemID, userCode, costCenterColumn, jobStatusColumn, empStatusColumn);
            if (reportType)
            {
                query = query.Replace("'", "''");
            }
        ...
        {
            string strPayCalcTable = "T_EmployeePayrollCalc";
            DALHelper dal = new DALHelper();
            DataSet ds = dal.ExecuteDataSet(
                            string.Format(@"
                            select Ppm_CycleIndicator, Ppm_CycleIndicatorSpecial from T_PayPeriodMaster
                            where Ppm_PayPeriod = '{0}'", strPayPeriod));
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    if (isSepTable == true)
                        strPayCalcTable = "T_EmployeePayrollCalcSep";
                    else
                        strPayCalcTable = "T_EmployeePayrollCalc";
                }
                else
                {
                    if (isSepTable == true)
                        strPayCalcTable = "T_EmployeePayrollCalcSepHist";
                    else
                    {
                        ...
                        {
                            strPayCalcTable = "T_EmployeePayrollCalcAnnual"; //Within the same year as the current
                        }
                        else
                        {
                            strPayCalcTable = "T_EmployeePayrollCalcHist"; //Not the same year with the current
                        }
                    }
                }
            }
                ...
                switch (report)
                {
                    ...
                    case "Overtime Report":
                        ...
                        SELECT Eot_EmployeeId as IDNumber
		                        ...
		                        , CASE WHEN UserMaster2.Umt_Usercode IS NULL THEN Eot_CheckedBy ELSE Rtrim(UserMaster2.Umt_userlname)  + '', '' + RTrim(UserMaster2.Umt_userfname) END AS Checker1Name
		                        , Eot_CheckedDate as CheckDate1
		                        , Eot_Checked2By as Checker2
		                        , CASE WHEN UserMaster3.Umt_Usercode IS NULL THEN Eot_Checked2By ELSE Rtrim(UserMaster3.Umt_userlname)  +  '', '' + RTrim(UserMaster3.Umt_userfname) END AS Checker2Name
		                        , Eot_Checked2Date as CheckDate2
		                        , Eot_ApprovedBy as Approver
		                        , CASE WHEN UserMaster4.Umt_Usercode IS NULL THEN Eot_ApprovedBy ELSE Rtrim(UserMaster4.Umt_userlname)  + '', '' + RTrim(UserMaster4.Umt_userfname) END AS ApproverName  
                        ...
                        LEFT JOIN  {0}..T_UserMaster AS UserMaster2 ON UserMaster2.Umt_Usercode = Eot_CheckedBy
                        LEFT JOIN  {0}..T_UserMaster AS UserMaster3 ON UserMaster3.Umt_Usercode = Eot_Checked2By
                        LEFT JOIN  {0}..T_UserMaster AS UserMaster4 ON UserMaster4.Umt_Usercode = Eot_ApprovedBy 
                        {1}
                    ...
                    case "Leave Availment Report":
                        ...
                        SELECT Elt_EmployeeId as IDNumber
		                        ...
		                        , CASE WHEN UserMaster2.Umt_Usercode IS NULL THEN Elt_CheckedBy ELSE Rtrim(UserMaster2.Umt_userlname)  + '', '' + RTrim(UserMaster2.Umt_userfname) END AS Checker1Name
		                        , Elt_CheckedDate as CheckDate1
		                        , Elt_Checked2By as Checker2
		                        , CASE WHEN UserMaster3.Umt_Usercode IS NULL THEN Elt_Checked2By ELSE Rtrim(UserMaster3.Umt_userlname)  +  '', '' + RTrim(UserMaster3.Umt_userfname) END AS Checker2Name
		                        , Elt_Checked2Date as CheckDate2
		                        , Elt_ApprovedBy as Approver
		                        , CASE WHEN UserMaster4.Umt_Usercode IS NULL THEN Elt_ApprovedBy ELSE Rtrim(UserMaster4.Umt_userlname)  + '', '' + RTrim(UserMaster4.Umt_userfname) END AS ApproverName  
                        ...
                        LEFT JOIN  {0}..T_AccountDetail  CycIndic ON CycIndic.Adt_AccountCode = Elt_LeaveFlag
	                        AND CycIndic.Adt_AccountType = ''CYCLEINDIC''
                        LEFT JOIN  {0}..T_UserMaster AS UserMaster2 ON UserMaster2.Umt_Usercode = Elt_CheckedBy
                        LEFT JOIN  {0}..T_UserMaster AS UserMaster3 ON UserMaster3.Umt_Usercode = Elt_Checked2By
                        LEFT JOIN  {0}..T_UserMaster AS UserMaster4 ON UserMaster4.Umt_Usercode = Elt_ApprovedBy 
                    ...
                    case "DTR Report":
                        #region query
                        query = @"
                        SELECT Dtr_EmployeeID AS IDNumber
	                        , Emt_LastName  + '', '' + Emt_FirstName + '' '' +  Left(Emt_Middlename,1 ) + ''.'' as Fullname
	                        , Emt_CostCenterCode + '' - '' + dbo.getCostCenterFullNameV2(Emt_CostCenterCode) as Costcenter
	                        , CONVERT(date, Dtr_LogDate) AS LogDate
	                        , Dtr_LogTime AS LogTime
	                        , Dtr_LogType AS LogType
	                        , Dtr_StationNo AS StationNumber  
	                        , PayrollType.Adt_AccountDesc as PayrollType 
	                        , EmploymentStatus.Adt_AccountDesc AS EmploymentStatus
	                        , JobStatus.Adt_AccountDesc AS JobStatus
                        FROM  {0}..T_DTR DTR
                        INNER JOIN {1}..T_EmployeeMaster ON Emt_EmployeeID = Dtr_EmployeeID  
                ...
                switch (report)
                {
                    ...
                    case "Overtime Report":
                        ...
                        SELECT 	 Eot_EmployeeId as [ID Number]
		                        ...
		                        , CASE WHEN UserMaster2.Umt_Usercode IS NULL THEN Eot_CheckedBy ELSE Rtrim(UserMaster2.Umt_userlname)  + ', ' + RTrim(UserMaster2.Umt_userfname) END AS [Checker 1 Name]
		                        , Eot_CheckedDate as [Check Date 1]
		                        , Eot_Checked2By as [Checker 2]
		                        , CASE WHEN UserMaster3.Umt_Usercode IS NULL THEN Eot_Checked2By ELSE Rtrim(UserMaster3.Umt_userlname)  +  ', ' + RTrim(UserMaster3.Umt_userfname) END AS [Checker 2 Name]
		                        , Eot_Checked2Date as [Check Date 2]
		                        , Eot_ApprovedBy as [Approver]
		                        , CASE WHEN UserMaster4.Umt_Usercode IS NULL THEN Eot_ApprovedBy ELSE Rtrim(UserMaster4.Umt_userlname)  + ', ' + RTrim(UserMaster4.Umt_userfname) END AS [Approver Name]  
                    ...
                    case "Leave Availment Report":
                        ...
                        SELECT 	 Elt_EmployeeId as [ID Number]
		                        ...
		                        , CASE WHEN UserMaster2.Umt_Usercode IS NULL THEN Elt_CheckedBy ELSE Rtrim(UserMaster2.Umt_userlname)  + ', ' + RTrim(UserMaster2.Umt_userfname) END AS [Checker 1 Name]
		                        , Elt_CheckedDate as [Check Date 1]
		                        , Elt_Checked2By as [Checker 2]
		                        , CASE WHEN UserMaster3.Umt_Usercode IS NULL THEN Elt_Checked2By ELSE Rtrim(UserMaster3.Umt_userlname)  +  ', ' + RTrim(UserMaster3.Umt_userfname) END AS [Checker 2 Name]
		                        , Elt_Checked2Date as [Check Date 2]
		                        , Elt_ApprovedBy as [Approver]
		                        , CASE WHEN UserMaster4.Umt_Usercode IS NULL THEN Elt_ApprovedBy ELSE Rtrim(UserMaster4.Umt_userlname)  + ', ' + RTrim(UserMaster4.Umt_userfname) END AS [Approver Name]  
                    ...
                    case "DTR Report":
                        #region query
                        query = @"
                        SELECT Dtr_EmployeeID AS  [ID Number]
	                        , Emt_LastName AS [Last Name]
	                        , Emt_FirstName AS [First Name]
	                        , CONVERT(date, Dtr_LogDate) AS [Log Date]
	                        , Dtr_LogTime AS [Log Time]
	                        , Dtr_LogType AS [Log Type]
	                        , Dtr_StationNo AS [Station Number]  
	                        , PayrollType.Adt_AccountDesc as [Payroll Type] 
	                        , EmploymentStatus.Adt_AccountDesc AS [Employment Status]
	                        , JobStatus.Adt_AccountDesc AS [Job Status]
                        FROM  {0}..T_DTR DTR
                    ...
                    case "Tardiness/Undertime/Absences Report":
                        #region query
                        query = @"
                            SELECT Ell_EmployeeId as [ID Number]
	                            , {0}..T_EmployeeMaster.Emt_Lastname as [Last Name]  
	                            , {0}..T_EmployeeMaster.Emt_Firstname as [First Name]  
	                            , {0}..T_EmployeeMaster.Emt_MiddleName as [Middle Name]
	                            , CONVERT(VARCHAR(10),Ell_ProcessDate,101) as [Date]  
                                , Ell_AbsentHour as [Absent]
	                            , CASE WHEN Umt_Usercode is null THEN {0}..T_EmployeeLogLedger.Usr_login ELSE Rtrim(Umt_userlname) + ' , ' + RTrim(Umt_userfname) END as [Last User Modified]    
	                            , CONVERT(VARCHAR(10),{0}..T_EmployeeLogLedger.ludatetime,101) as [Last Modified Date]
                             FROM {0}..T_EmployeeLogLedger
                            LEFT JOIN {0}..T_UserMaster on  Umt_Usercode = {0}..T_EmployeeLogLedger.Usr_login    
                            INNER JOIN {0}..T_EmployeeMaster on Emt_EmployeeID = Ell_EmployeeID
                            {1}
                            ...
                            SELECT Ell_EmployeeId as [ID Number]
	                            ...
	                            , CONVERT(VARCHAR(10),Ell_ProcessDate,101) as [Date]  
                                , Ell_AbsentHour as [Absent]
	                            , CASE WHEN Umt_Usercode is null THEN {0}..T_EmployeeLogledgerHist.Usr_login ELSE Rtrim(Umt_userlname) + ' , ' + RTrim(Umt_userfname) END as [Last User Modified]    
	                            , CONVERT(VARCHAR(10),{0}..T_EmployeeLogledgerHist.ludatetime,101) as [Last Modified Date]
                             FROM {0}..T_EmployeeLogledgerHist
                            LEFT JOIN {0}..T_UserMaster on  Umt_Usercode = {0}..T_EmployeeLogledgerHist.Usr_login    
                            INNER JOIN {0}..T_EmployeeMaster on Emt_EmployeeID = Ell_EmployeeID
                                                        {1}
                        ";
                        #endregion
                    ...
                    case "Loan Collection Report":
                        #region query
                        query = @"
                            SELECT  Prm_Cycle AS [Year & Month]
		                        , Prm_DeductionCode as [Deduction Code] 
		                        , Prm_EmployeeID as [ID Number]
		                        , Emt_LastName as [Last Name]
		                        , Emt_FirstName as [First Name]
		                        , Emt_MiddleName as [Middle Name]
		                        , Emt_MaidenName as [Maiden Name]
		                        , Prm_EE as [Amount]
		                        , ISNULL(SUBSTRING(Prm_Filler, 13,10), '') as [Check Date]
		                        , ISNULL(SUBSTRING(Prm_Filler, 23,10), '') as [Loan Amount]
		                        , ISNULL(SUBSTRING(Prm_Filler, 33,10), '') as [Principal Amount]		
		                        , ISNULL(SUBSTRING(Prm_Filler, 1,2), '') as [Status Code]
		                        , ISNULL(SUBSTRING(Prm_Filler, 3,10), '') as [Status Date]
		                        , CASE WHEN Umt_Usercode is null THEN {0}..T_PremiumLoanRemittance.Usr_login ELSE Rtrim(Umt_userlname) + ' , ' + RTrim(Umt_userfname) END as [Last User Modified]    
		                        , CONVERT(VARCHAR(10),{0}..T_PremiumLoanRemittance.ludatetime,101) as [Last Modified Date]
		                        , Emt_SSSNo as [SSS Number]
		                        , Emt_PhilhealthNo as [Philhealth Number]
		                        , Emt_HDMFNo as [Pag-ibig Number]
		                        , JobStatus.adt_accountdesc as [JobStatus] 
		                        , PayrollType.adt_accountdesc as [Payroll Type]
		                        , EmploymentStatus.adt_accountdesc as [Employment Status]
		                        , Emt_Gender as [Gender]  
                                , Emt_CostCenterCode as [Costcenter]
                                , dbo.getCostCenterFullNameV2(Emt_costcentercode) as [Description] 
                        FROM {0}..T_PremiumLoanRemittance
                        LEFT JOIN {0}..T_UserMaster on  Umt_Usercode = {0}..T_PremiumLoanRemittance.Usr_login    
                        INNER JOIN {0}..T_EmployeeMaster on Emt_EmployeeID = Prm_EmployeeID
                        LEFT JOIN {0}..T_AccountDetail PayrollType on PayrollType.adt_accountcode = Emt_PayrollType  
                         and  PayrollType.adt_accounttype = 'PAYTYPE'  
                        LEFT JOIN {0}..T_AccountDetail JobStatus on JobStatus.adt_accountcode = Emt_JobStatus  
                         and  JobStatus.adt_accounttype = 'JOBSTATUS'  
                        LEFT JOIN {0}..T_AccountDetail EmploymentStatus on EmploymentStatus.adt_accountcode = Emt_EmploymentStatus  
                         and  EmploymentStatus.adt_accounttype = 'EMPSTAT'  
                        INNER JOIN {0}..T_DeductionCodeMaster on Dcm_DeductionCode = Prm_DeductionCode
	                        and Dcm_DeductionType like 'L%'
                    ...
                    case "13th Month Report":
                        ...
                        query = @"
                            ...
                            SELECT Ebn_EmployeeId as [ID Number]
                             , Emt_LastName as [Last Name]
                             , Emt_FirstName as [First Name]
                             , Emt_MiddleName as [Middle Name]
                             , Emt_MaidenName as [Maiden Name]
                             , Ebn_Payperiod as [Pay period]
                             , Ebn_SalaryRate as [Salary Rate]
                             , PayrollType.Adt_AccountDesc as [Payroll Type]
                             , Ebn_RegularAmt as [Regular Pay]
                             , Ebn_AdjustmentAmt as [Adjustment Amount]
                             , Ebn_BonusAmt as [Bonus Amount]
                             , Ebn_UsedNontaxAmt as [Used Nontax Amount]
                             , Ebn_NontaxAmt as [Nontax Amount]
                             , Ebn_TaxAmt as [Tax Amount]
                             , CASE WHEN Ebn_Post = 1  then 'YES' Else 'NO' End as [Post to Payroll]
                             , Ebn_Costcenter as [Costcenter]
                             , {0}.dbo.getCostCenterFullNameV2(Ebn_Costcenter) as [Description]
                             , Ebn_ActualDays as [Actual Days]
                             , CASE WHEN Umt_Usercode is null THEN EB.Usr_login ELSE Rtrim(Umt_userlname) + ' , ' + RTrim(Umt_userfname) END as [Last User Modified]    
                             , CONVERT(VARCHAR(10),EB.ludatetime,101) as [Last Modified Date]
                             , JobStatus.adt_accountdesc as 'JobStatus' 
                             , EmploymentStatus.adt_accountdesc as 'Employment Status'
                             , Emt_Gender as 'Gender'  
                            FROM {0}..T_EmployeeBonus EB
                            LEFT JOIN {0}..T_UserMaster on  Umt_Usercode = EB.Usr_login    
                            INNER JOIN {0}..T_EmployeeMaster on Emt_EmployeeID = Ebn_EmployeeId
                            LEFT JOIN {0}..T_AccountDetail PayrollType on PayrollType.adt_accountcode = Ebn_PayrollType  
                             and  PayrollType.adt_accounttype = 'PAYTYPE'  
                            LEFT JOIN {0}..T_AccountDetail JobStatus on JobStatus.adt_accountcode = Emt_JobStatus  
                             and  JobStatus.adt_accounttype = 'JOBSTATUS'  
                            LEFT JOIN {0}..T_AccountDetail EmploymentStatus on EmploymentStatus.adt_accountcode = Emt_EmploymentStatus  
                            ...
                            SELECT Ebn_EmployeeId as [ID Number]
                             ...
                             , Ebn_ActualDays as [Actual Days]
                             , CASE WHEN Umt_Usercode is null THEN EB.Usr_login ELSE Rtrim(Umt_userlname) + ' , ' + RTrim(Umt_userfname) END as [Last User Modified]    
                             , EB.ludatetime as [Last Modified Datetime]
                             , JobStatus.adt_accountdesc as 'JobStatus' 
                             , EmploymentStatus.adt_accountdesc as 'Employment Status'
                             , Emt_Gender as 'Gender'  
                            FROM {0}..T_EmployeeBonusHist EB
                            LEFT JOIN {0}..T_UserMaster on  Umt_Usercode = EB.Usr_login    
                            INNER JOIN {0}..T_EmployeeMaster on Emt_EmployeeID = Ebn_EmployeeId
                            LEFT JOIN {0}..T_AccountDetail PayrollType on PayrollType.adt_accountcode = Ebn_PayrollType  
                             and  PayrollType.adt_accounttype = 'PAYTYPE'  
                ...
                ";
                    ...
                    case "Premium Remittance Report":
                        ...
                        SELECT  Prm_Cycle AS [Year & Month]
		                    ...
		                    , Emt_MiddleName as [Middle Name]
		                    , Emt_MaidenName as [Maiden Name]
		                    , Prm_ER as [Employer Share]
		                    , Prm_EE as [Employee Share]
		                    , Prm_SalaryCredit as [Salary Credit]
		                    , ISNULL(SUBSTRING(Prm_Filler, 1,2), '') as [Status Code]
		                    , ISNULL(SUBSTRING(Prm_Filler, 3,10), '') as [Status Date]
		                    , CASE WHEN Umt_Usercode is null THEN T_PremiumLoanRemittance.Usr_login ELSE Rtrim(Umt_userlname) + ' , ' + RTrim(Umt_userfname) END as [Last User Modified]    
		                    , CONVERT(VARCHAR(10),{0}..T_PremiumLoanRemittance.ludatetime,101) as [Last Modified Date]
		                    ,case when  Emt_SSSNo='' then 'N/A' else Emt_SSSNo end as [SSS Number]
                            ,case when Emt_PhilhealthNo='' then 'N/A' else Emt_PhilhealthNo end as [Philhealth Number]
                            ,case when Emt_HDMFNo='' then 'N/A' else Emt_HDMFNo end as [Pag-ibig Number]
                            ,case when Emt_TIN='' then 'N/A' else Emt_TIN end as [TIN]
                            , JobStatus.adt_accountdesc as [JobStatus]
		                    , PayrollType.adt_accountdesc as [Payroll Type]
		                    , EmploymentStatus.adt_accountdesc as [Employment Status]
		                    , Emt_Gender as [Gender]  
                    ...
                    LEFT JOIN {0}..T_AccountDetail EmploymentStatus on EmploymentStatus.adt_accountcode = Emt_EmploymentStatus  
                     and  EmploymentStatus.adt_accounttype = 'EMPSTAT'  
                    INNER JOIN {0}..T_DeductionCodeMaster on Dcm_DeductionCode = Prm_DeductionCode
	                    and Dcm_DeductionType like 'G%'
                      
                      {1}
                    And (Prm_EE >0 and Prm_ER >0)
                        ";
                        #endregion
                        break;
                    case "Withholding Tax Report":
                        ...
                        query = @"
                            select 
                             Epc_EmployeeId [Employee ID]
                             ,Emt_LastName [Last Name]
                             ,Emt_FirstName [First Name]
                             ,Emt_MiddleName [Middle Name]
                             ,Epc_Costcenter [Costcenter]
                             ,dbo.getCostCenterFullNameV2(Epc_Costcenter) [Description]
                             ,Emt_TIN [TIN]
                             ,Epc_CurrentPayPeriod [Payperiod]
                             ,Epc_TaxCode [Tax Code]
                             ,Epc_WTaxAmt [Witholding Tax Amount]
                             
                            from {0}..T_EmployeePayrollCalc
                            left join {0}..T_EmployeeMaster on Emt_EmployeeID = Epc_EmployeeId
                            {1}
                            ...
                            from {0}..T_EmployeePayrollCalcAnnual
                            left join {0}..T_EmployeeMaster on Emt_EmployeeID = Epc_EmployeeId
                             {1}       
                            ...
                            from {0}..T_EmployeePayrollCalcHist
                            left join {0}..T_EmployeeMaster on Emt_EmployeeID = Epc_EmployeeId
        ...
        public DataTable GetEmployeeList2(string[] dbs)
        {
            string query = @"
               		 select 
				        Emt_EmployeeID [ID],
        ...
        public DataTable GetRankLevel()
        {
            string query = @"
                    select 
                        ...
                        Adt_AccountDesc [Description]	 
                    from T_AccountDetail
                    where Adt_AccountType = 'RANKLEVEL'
                                  AND Adt_Status = 'A'
                    ";
            ...
            if (Transaction == "OVERTIME")
            {
                ...
                if (routeNumber == 1)
                {
                    query = @"
                        ...
                        from T_EmployeeOvertime
                        left outer join T_UserMaster
                        on Eot_CheckedBy = Umt_UserCode
                ...
                else if (routeNumber == 2)
                {
                    query = @"
                       select distinct
                        ...
                        from T_EmployeeOvertime
                        left outer join T_UserMaster
                        on Eot_Checked2By = Umt_UserCode
                ...
                else if (routeNumber == 3)
                {
                    query = @"
                       select distinct
                        ...
                        from T_EmployeeOvertime
                        left outer join T_UserMaster
                        on Eot_ApprovedBy = Umt_UserCode
                ...
                else if (routeNumber == 2)
                {
                    query = @"
                       select distinct
                        ...
                        from T_EmployeeLeaveAvailment
                        left outer join T_UserMaster
                        on Elt_Checked2By = Umt_UserCode
                ...
                else if (routeNumber == 3)
                {
                    query = @"
                       select distinct
                        ...
                        from T_EmployeeLeaveAvailment
                        left outer join T_UserMaster
                        on Elt_ApprovedBy = Umt_UserCode
        ...
        public string GetPayrollScheduleDataSummary2(string reportType, string Cols)
        {
            string queryColumns = string.Empty;
            ...
            switch (reportType)
            {
                case "HOURS":
                    ...
                    '@Prof' [Profiles]
                    , @COLS
                    , SUM(Epc_AbsentHr) as [Hours Late/UT/ Absent]
                    , SUM(Epc_RegularHr) as [Hrs. Regular]
                    , SUM(Epc_RegularOTHr) as [OT Hrs. Regular]
                    , SUM(Epc_RestdayHr) as [OT Hrs. Rest Day 1st 8]
                    , SUM(Epc_RestdayOTHr) as [OT Hrs. Rest Day Excess 8]
                    , SUM(Epc_SpecialHolidayHr )as [OT Hrs. Spec Hol 1st 8]
                    , SUM(Epc_SpecialHolidayOTHr)as [OT Hrs. Spec Hol Excess 8]
                    , SUM(Epc_LegalHolidayHr )as [OT Hrs. Legal Hol 1st 8]
                    , SUM(Epc_LegalHolidayOTHr )as [OT Hrs. Legal Hol Excess 8]
                    , SUM(Epc_RestdaySpecialHolidayHr )as [OT Hrs. SH Fall on RD 1st 8]
                    , SUM(Epc_RestdaySpecialHolidayOTHr )as [OT Hrs. SH Fall on RD Excess 8]
                    , SUM(Epc_RestdayLegalHolidayHr )as [OT Hrs. LH Fall on RD 1st 8]
                    , SUM(Epc_RestdayLegalHolidayOTHr )as [OT Hrs. LH Fall on RD Excess 8]
                    , SUM(Epc_RegularNDHr + Epc_RegularOTNDHr )as [NDOT Hrs Regular]
                    , SUM(Epc_RestdayNDHr )as [NDOT Hrs. Rest Day 1st 8]
                    , SUM(Epc_RestdayOTNDHr )as [NDOT Hrs. Rest Day Excess 8]
                    , SUM(Epc_SpecialHolidayNDHr )as [NDOT Hrs. Spec Hol 1st 8]
                    , SUM(Epc_SpecialHolidayOTNDHr )as [NDOT Hrs. Spec Hol Excess 8]
                    , SUM(Epc_LegalHolidayNDHr )as [NDOT Hrs. Legal Hol 1st 8]
                    , SUM(Epc_LegalHolidayOTNDHr )as [NDOT Hrs. Legal Hol Excess 8]
                    , SUM(Epc_RestdaySpecialHolidayNDHr )as [NDOT Hrs. SH Fall on RD 1st 8]
                    , SUM(Epc_RestdaySpecialHolidayOTNDHr )as [NDOT Hrs. SH Fall on RD Excess 8]
                    , SUM(Epc_RestdayLegalHolidayNDHr )as [NDOT Hrs. LH Fall on RD 1st 8]
                    , SUM(Epc_RestdayLegalHolidayOTNDHr )as [NDOT Hrs LH Fall on RD Excess 8]
                ...
                case "AMOUNTS":
                    ...
                    , SUM(Epc_RegularAmt )as [Amt. Regular]
                ...
                case "DEDUCTIONS":
                   ...
                   -- , SUM(ISNULL(LIFESTYLE, 0)  )as [LIFE STYLE LOAN]
                    ...
                    , SUM(Epc_OtherDeductionAmt) [Other Deduction]
            ...
            queryColumns = queryColumns.Replace("@COLS", Cols);

            return queryColumns;

        }

        public DataTable GetDeductionLoan()
        {
            string query = @"
                    select 
	                    Dcm_DeductionCode [Deduction Code],
        ...
        public DataTable GetYearMonthPremiumRemit()
        {
            string query = @"
               select 
                    Distinct 
                        Prm_Cycle [Cycle],
                        Datename(month, (RIGHT(Prm_Cycle, 2)) + '/01/2001' ) [Month] ,
                        Left(Prm_Cycle, 4) [Year]
                    from T_PremiumLoanRemittance
                    order by [Cycle] desc
                    ";
        ...
        public DataTable GetEmployeeListWithAccess(string SystemID, string UserCode)
        {
            ...
            varparams[3] = cbl.GetCostCenterAccess(SystemID).ToString();

            query = string.Format(CommonConstants.Queries.SelectIDNumber, varparams);

            query = @"Select [ID Number], [Last Name], [First Name], [MI], [Job Status]
                        From (" + query + "\n) T ORDER BY [Last Name] ASC";
        ...
        public DataTable GetDeductionPremium()
        {
            string query = @"
                    select 
	                    Dcm_DeductionCode [Deduction Code],
        ...
        public DataTable GetEmploymentStatus(string userlogged, string SystemID)
        {
            string cond = string.Empty;
            ...
            string query = @"
               		 SELECT [Adt_AccountCode] AS 'Code'
                                   ,[Adt_AccountDesc] AS 'Description'
                              FROM  [T_AccountDetail]
                             WHERE  [Adt_Status] = 'A'
                               AND  [Adt_AccountType] = 'EMPSTAT'
            ...
            using (DALHelper dal = new DALHelper())
            {
                    ...
                    && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    query = string.Format(query, string.Format(" and Adt_AccountCode in ({0})", Cols));
        ...
        public DataTable GetJobStatus(string userlogged, string SystemID)
        {
            string cond = string.Empty;
        ...
        public DataTable GetTaxExemptStat()
        {
            string query = @"
                    select 
                        Adt_AccountCode [Code],
        ...
        public DataSet GetReportExclusion(string strFlexQuery)//Added for filtering Applied,Null value and O Contribution
        {                                                                            // of Philhealth and Pag-ibig Added by Luis 11/15/12
            strFlexQuery = strFlexQuery.Replace("@FORFLEX", @"[Employer Share]='0.00' and [Employee Share]='0.00'");
            DataSet dtResult;
            using (DALHelper dal = new DALHelper())
            {
                dal.OpenDB();
                dtResult = dal.ExecuteDataSet(strFlexQuery);
                dal.CloseDB();
            }
            return dtResult;
        }
